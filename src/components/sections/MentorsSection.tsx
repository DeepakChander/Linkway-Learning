...
function InstructorCard({ instructor, index }: { instructor: typeof INSTRUCTORS[number]; index: number }) {
    ...
    return (
        <div className="group relative w-[240px] md:w-[260px] shrink-0 rounded-2xl bg-white border border-gray-100 overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300">
            {/* Top accent */}
            <div className={`h-1 bg-gradient-to-r ${color.gradient}`} />

            {/* Instructor image */}
            <div className={`relative w-full aspect-[5/4] ${color.bg} overflow-hidden`}>
                <Image
            ...
            {/* Content */}
            <div className="p-3">
                <h3 className="text-sm font-bold text-navy-900 leading-tight mb-0.5">
                    {instructor.name}
                </h3>
                ...
                <div className="flex flex-wrap gap-1">
                    {instructor.specializations.map((spec, j) => (
                        <span
                            key={j}
                            className={`px-1.5 py-0.5 text-[10px] font-medium rounded-full ${color.badge}`}
                        >
                            {spec}
                        </span>
                    ))}
...
export default function MentorsSection() {
    ...
    return (
        <ThemeProvider theme="light">
            <section ref={sectionRef} className="py-12 md:py-16 overflow-x-clip relative" style={{ backgroundColor: "#f2f1ee" }}>
